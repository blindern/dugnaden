...
------------------------------------------------------------------------------------------ */
define(DUGNADURL, "http://www.blindern-studenterhjem.no/dugnaden/");
...
if(!strcmp($_SERVER['DOCUMENT_ROOT'], "/Library/WebServer/Documents"))
{
	// LOCAL MAC
	@include_once "./txt/dbinfo_local.txt";
	@include_once "../txt/dbinfo_local.txt";
	
	$append_myphpadmin_link = "<p class=\"footer_info\"><a href=\"http://www.blindern-studenterhjem.no/dugnaden/\">Dugnaden</a><br /><a href=\"http://phpmyadmin.webdeal.no/\">Dugnaden MyPHP Admin</a></p>";
	...
	switch($formdata["admin"])
	{
		...
		case "Annulere bot":
			...
			if(@mysql_num_rows($result))
			{
				while($row = @mysql_fetch_array($result) )
				{
					...
					if(!$row["bot_annulert"]) $bot_count++; else $annulert_count++;
		...
		case "Innkalling av nye":
			...
			if($valid_login == 1)
			{
				...
				$result_deltager = mysql_query($query);
				...
				if(!empty($formdata["list"]))
				{
					...
					if(!strcmp($formdata["delappend"], "del"))
					{
						...
						if(truncateAllowed())
						{
							...
							$dugnad_result = @mysql_query($dugnad_query);
...
function insert_dugnad_using_id($beboer_id, $dugnad_id, $type = 1, $notat = null)
{
	...
	{
		$query = "INSERT INTO bs_deltager (deltager_beboer, deltager_dugnad, deltager_type, deltager_notat)
						VALUES('". $beboer_id ."', '". $dugnad_id ."', '". $type ."', '". $notat ."')";
...
function smart_create_dugnad($beboer_id)
{
	...
	if(!empty($empty_dugnad["key"]) && valid_dugnad_id($empty_dugnad["key"]) && !allready_added_dugnad($id, $empty_dugnad["key"]) )
	{
		$query = "INSERT INTO bs_deltager (deltager_beboer, deltager_dugnad, deltager_type, deltager_notat)
					VALUES('". $id ."', '". $date ."', '1', 'Opprettet dugnad.')";
	...
	else
	{
		...
		if(!allready_added_dugnad($id, $date) )
		{
		...
		else
		{
			...
			insert_note($id, "Overf&oslash;re dugnad til ". $semester . $year .".");
			return false;
		}
	}
}
...
function get_dugnads($id, $hide_outdated_dugnads = false)
{
	...
	$query = "SELECT	deltager_dugnad	AS id,
				...
				WHERE deltager_beboer = '". $id ."'
					". ($hide_outdated_dugnads ? 'AND (deltager_gjort = \'0\' AND dugnad_dato > now()) ' : '') ."
...
function output_full_list($admin = false)
{
	...
	if(empty($sort_date) )
	{
		...
		-------------------------------------------------------- */
		$c = 0;
		$result = @mysql_query($query . $sort_query);
...
/* ******************************************************************************************** *
 *  file_to_array($filename, $strip = false, $buffer = null)
 * --------------------------------------------------------------------------------------------
 *
 * ============================================================================================ */

function file_to_array($filename, $strip = false, $buffer = null)
{
	global $path_final, $using_layout;
	
	$using_layout = $filename;
	
	// Opens and reads line by line and returns an array with content and variable:
	// Content being the html to display, and variable is the interactive content.

	if(empty($buffer) )
	{
		if (!($fp = fopen($path_final . $filename, "r")))  
		{
			exit ("<p>Please come back later, the server is currently busy.</p><p>$filename was unavailable.</p>\n<p>Sorry,<br \>\n   The PWP Staff</p>");
		}
		
		$count = 0;
			
		// Read entire file into a single string buffer skipping empty
		// lines and comments (usefull only for parsing config files).
		
		while (!feof($fp))
		{
			$tmp = fgets($fp, 1024);
			if (strlen($tmp) > 2 && strcmp(substr($tmp, 0, 2), "//"))
				$buffer .= ($strip ? trim($tmp) : $tmp);
		}
	...
	$out = preg_split('(\[([^]]+?)\])', $buffer, -1, PREG_SPLIT_DELIM_CAPTURE);

	$final = array();

	if (!$strip) $final["head"] = $out[0];
