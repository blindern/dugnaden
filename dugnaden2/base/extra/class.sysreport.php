...
class sysreport
{
	 ...
	 */
	public static function log($data, $title = "BlindernStud Email Log")
	{
		global $_base, $up_id;
...
'.$stack_text;
		...
		self::log($text, "StreetzMafia Fanget Exception".($subtitle == "" ?  "" : " [$subtitle]"));
	}
...
<pre>'.htmlspecialchars(mysql_result($result, 0, "Status")).'</pre>';
		...
		// send e-post og logg
		if (!self::log($text, "StreetzMafia Exception"))
		{
